namespace :ptourist do
  ...
  def create_business organizer, bus
    puts "building business for #{bus[:offer]}, by #{organizer.name}"
    bus=Business.create(:creator_id=>organizer.id,:offer=>bus[:offer],:description=>bus[:description])
    organizer.add_role(Role::ORGANIZER, bus).save
  end
...
"}
    ...
    organizer=get_user("peter")
    bus= {:offer=>"First offer by Peter",
     :description=>"Nice one"
     }
    create_business organizer, bus

    organizer=get_user("jan")
    bus= {:offer=>"First offer by Jan",
